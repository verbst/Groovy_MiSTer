		...
		case STRINGARR:
			{
				...
				// empty value ("key=") clears the whole list, so a per-core section
				// can drop entries added by [MiSTer] instead of appending to them
				if (!buf[i])
				{
					memset(var->var, 0, (size_t)var->min * item_sz);
					break;
				}
